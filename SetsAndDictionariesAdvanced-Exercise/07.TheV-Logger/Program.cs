namespace _07.TheV_Logger
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var vlogger in theVlogger.OrderByDescending(v => v.Value.Followers.Count)
                .ThenBy(v => v.Value.Following.Count))
            {
            ...
            static bool CheckIfCanFollow(Dictionary<string, Vlogger> theVlogger, string firstVlogger, string secondVlogger)
            {
                bool returnValue = theVlogger.ContainsKey(firstVlogger)
